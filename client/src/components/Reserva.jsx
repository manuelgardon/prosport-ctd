...
export default function Reserva({ precio }) {
    ...
    useEffect(() => {
        async function obtenerReservaExistente() {
          try {
            const response = await axios.get(`http://54.219.42.160:8085/api/reservas?espacioId=${id}`);
        ...
        async function obtenerDiasDisponibles() {
          try {
            const response = await axios.get(`http://54.219.42.160:8085/api/espacios/${id}`);
        ...
        if (token) {
            ...
            try {
                ...
                const response = await axios.post(
                    'http://54.219.42.160:8085/api/reservas',
                    {
                        espacioId: id,
