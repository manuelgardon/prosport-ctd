...
export default function Espacios({ espacios, setEspacios, changeFilters, filtros }) {
    ...
    useEffect(() => {
        ...
        async function cargarEspacios() {
            try {
                const response = await axios.get(
                    `https://strongly-secure-kiwi.ngrok-free.app/api/espacios?pagina=${pagina}&porPagina=${espaciosPorPagina}&deporte=${filtros.deporte}&ciudad=${filtros.ciudad}`
                )
        ...
        async function cargarFavoritos() {
            ...
            try {
                const responseFavoritos = await axios.get(`https://strongly-secure-kiwi.ngrok-free.app/api/user/favoritos`, { withCredentials: true })
    ...
    async function agregarFavorito(espacio) {
        if (token) {
            try {
                const response = await axios.post('https://strongly-secure-kiwi.ngrok-free.app/api/favoritos', { espacioId: espacio._id }, { withCredentials: true })
    ...
    async function eliminarFavorito(espacio) {
        if (token) {
            try {
                const response = await axios.delete(`https://strongly-secure-kiwi.ngrok-free.app/api/favoritos/${espacio._id}`, { withCredentials: true })
                if (response.status === 200) {
