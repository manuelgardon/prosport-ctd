...
export default function Espacios({ espacios, setEspacios, changeFilters, filtros }) {
    ...
    useEffect(() => {
        ...
        async function cargarEspacios() {
            try {
                const response = await axios.get(
                    `http://54.219.42.160:8085/api/espacios?pagina=${pagina}&porPagina=${espaciosPorPagina}&deporte=${filtros.deporte}&ciudad=${filtros.ciudad}`
                )
        ...
        async function cargarFavoritos() {
            ...
            try {
                const responseFavoritos = await axios.get(`http://54.219.42.160:8085/api/user/favoritos`, { withCredentials: true })
    ...
    async function agregarFavorito(espacio) {
        if (token) {
            try {
                const response = await axios.post('http://54.219.42.160:8085/api/favoritos', { espacioId: espacio._id }, { withCredentials: true })
    ...
    async function eliminarFavorito(espacio) {
        if (token) {
            try {
                const response = await axios.delete(`http://54.219.42.160:8085/api/favoritos/${espacio._id}`, { withCredentials: true })
                if (response.status === 200) {
