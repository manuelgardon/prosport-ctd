        ...
        <div className="relative">
            <section className="grid gap-2 grid-cols-[2fr_1fr] rounded-2xl overflow-hidden">
                <div>
                    {espacio.fotos?.[0] && (
                        //foto principal
                        <img className="aspect-square object-cover cursor-pointer" src={`http://18.144.53.6:1234/${espacio.fotos[0]}`} alt=""
                            onClick={() => setRenderFotos(true)} />
                    )}
                </div>
                <div className="grid">
                    {espacio.fotos?.[1] && (
                        <img className="aspect-square object-cove cursor-pointer" src={`http://18.144.53.6:1234/${espacio.fotos[1]}`} alt=""
                            onClick={() => setRenderFotos(true)} />
                    )}
                    <div className="overflow-hidden">
                        {espacio.fotos?.[2] && (
                            <img className="aspect-square object-cover cursor-pointer" src={`http://18.144.53.6:1234/${espacio.fotos[2]}`}alt=""
                                onClick={() => setRenderFotos(true)} />
                        )}
                    </div>
                </div>
