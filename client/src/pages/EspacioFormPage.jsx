...
function EspacioFormPage() {
    ...
    useEffect(() => {
        if (!id) return
        axios.get(`http://127.0.0.1:1234/api/espacios/${id}`)
            .then((response) => {
                const { data } = response
    ...
    async function addNewEspacio(e) {
        ...
        if (!id) {
            if (token) {
                try {
                    const response = await axios.post('http://127.0.0.1:1234/api/espacios', {
                        deporte,
                        nombre,
                        descripcion,
                        ciudad,
            ...
            try {
                const response = await axios.put('http://127.0.0.1:1234/api/espacios', {
                    id,
                    deporte,
                    nombre,
                    descripcion,
